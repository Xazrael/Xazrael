...
-- Ana Ayarlar
local function saveMainSettings(radiusText, spiralStepText, voteSelection, seqActive, mainSpecActive, lobbyActive, lobbyMode, lobbyAltCode)
...
local function loadMainSettings()
    local default = { radius = 50, spiral_step = 2, vote = "replay", sequence_system_active = true, main_special_active = true, lobby_auto_active = false, lobby_auto_mode = "Event", lobby_alt_code = "" }
...
local guiSuccess, guiError = pcall(function()
    ...
    local titleBar = Instance.new("TextLabel"); titleBar.Name = "TitleBar"; titleBar.Parent = mainFrame; titleBar.Size = UDim2.new(1, 0, 0, 30); titleBar.BackgroundColor3 = Color3.fromRGB(45, 45, 55); titleBar.Text = "Oto-Script Menüsü v28.5"; titleBar.Font = Enum.Font.SourceSansBold; titleBar.TextSize = 16; titleBar.TextColor3 = Color3.fromRGB(255, 255, 255)
    ...
    saveButton.MouseButton1Click:Connect(function() 
        ...
        saveMainSettings(radiusInputBox.Text, spiralStepInputBox.Text, sonucSecimi, sequenceSystemActive, mainAndSpecialActive, lobbyAutomationActive, lobbyNextMode, lobbyAlternativeCode) 
    end)
    ...
    local function createTaskFrame(taskData)
        local taskFrame = Instance.new("Frame"); taskFrame.Parent = taskListFrame; taskFrame.Size = UDim2.new(1, -10, 0, 120); taskFrame.BackgroundColor3 = Color3.fromRGB(50, 52, 64);
        ...
        local inputOriginX = Instance.new("TextBox"); inputOriginX.Name="inputOriginX"; inputOriginX.Parent = placeFrame; inputOriginX.Size=UDim2.new(0,60,0,25);inputOriginX.Position=UDim2.new(0,5,0,0);inputOriginX.Text = tostring(taskData.origin and taskData.origin.X or 0); inputOriginX.PlaceholderText = "Origin X"
        local inputOriginY = Instance.new("TextBox"); inputOriginY.Name="inputOriginY"; inputOriginY.Parent = placeFrame; inputOriginY.Size=UDim2.new(0,60,0,25);inputOriginY.Position=UDim2.new(0,70,0,0);inputOriginY.Text = tostring(taskData.origin and taskData.origin.Y or 0); inputOriginY.PlaceholderText = "Origin Y"
        local inputOriginZ = Instance.new("TextBox"); inputOriginZ.Name="inputOriginZ"; inputOriginZ.Parent = placeFrame; inputOriginZ.Size=UDim2.new(0,60,0,25);inputOriginZ.Position=UDim2.new(0,135,0,0);inputOriginZ.Text = tostring(taskData.origin and taskData.origin.Z or 0); inputOriginZ.PlaceholderText = "Origin Z"
    ...
    local function loadSequenceIntoEditor(sequenceData)
        ...
        local totalH = #listLayout.Parent:GetChildren() * 125
        taskListFrame.CanvasSize = UDim2.new(0,0,0,totalH)
    end
...
coroutine.wrap(function()
    while true do
        if not scriptAktif or (inSequenceArea and sequenceSystemActive) or sequenceEditorOpen or not mainAndSpecialActive then task.wait(1) continue end
