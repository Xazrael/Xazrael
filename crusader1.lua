--[[
    Oyun Otomasyon Script'i v29.2
    Yapan: Gemini
    Hata Düzeltmeleri: Gemini

    Özellikler:
    - YENİ (v29.2): "Mevcut Unit'leri Ekle" özelliği artık yönü sabit olarak (0, -1, 0) atayarak yerleştirme sorunlarını giderir.
    - YENİ (v29.1): "Mevcut Unit'leri Ekle" butonu artık sadece ID'si 15 karakter olan geçerli birimleri ekliyor.
    - YENİ (v29.1): Ana script butonu (AÇIK/KAPALI) kapatılıp açıldığında sıralama listesini otomatik olarak günceller.
    - YENİ (v29.0): Sıralama Editörü'ne "Mevcut Unit'leri Ekle" butonu eklendi.
...
local specialAreaStatusLabel, sequenceStatusLabel, sequenceNameLabel, sequenceProgressLabel, currentTaskLabel, lobbyStatusLabel, checkAndManageSequence
...
local guiSuccess, guiError = pcall(function()
    ...
    local titleBar = Instance.new("TextLabel"); titleBar.Name = "TitleBar"; titleBar.Parent = mainFrame; titleBar.Size = UDim2.new(1, 0, 0, 30); titleBar.BackgroundColor3 = Color3.fromRGB(45, 45, 55); titleBar.Text = "Oto-Script Menüsü v29.2"; titleBar.Font = Enum.Font.SourceSansBold; titleBar.TextSize = 16; titleBar.TextColor3 = Color3.fromRGB(255, 255, 255)
    ...
    mainActionButton.MouseButton1Click:Connect(function() 
        scriptAktif = not scriptAktif
        mainActionButton.Text = scriptAktif and "Script: AÇIK" or "Script: KAPALI"
        mainActionButton.BackgroundColor3 = scriptAktif and Color3.fromRGB(80, 200, 120) or Color3.fromRGB(220, 80, 80) 
        
        if scriptAktif then
            bildirimGoster("Script yeniden aktif edildi, sıralamalar güncelleniyor.")
            loadAllSequences()
    ...
    addCurrentUnitsButton.MouseButton1Click:Connect(function()
            ...
            if unit:IsA("Model") then
                ...
                if #baseId == 15 then
                    ...
                    local placeTaskData = {
                        ...
                        direction = { X = 0, Y = -1, Z = 0 } -- Yön sabitlendi
                    }
        ...
        if addedUnitCount > 0 then
            bildirimGoster(addedUnitCount .. " unit için Place ve Upgrade görevleri eklendi.")
        else
            bildirimGoster("Geçerli unit bulunamadı (ID'si 15 karakter olmalı).")
        end
    end)
...
coroutine.wrap(function()
    ...
    checkAndManageSequence = function()
